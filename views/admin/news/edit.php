            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" class="form-control bg-black text-light border-secondary" value="<?= htmlspecialchars($news->title) ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label text-light">Tóm tắt</label>
                        <textarea name="summary" class="form-control bg-black text-light border-secondary" rows="3"><?= htmlspecialchars($news->summary ?? '') ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label class="form-label text-light">Nội dung chi tiết</label>
                        <textarea name="content" class="form-control bg-black text-light border-secondary" rows="15"><?= htmlspecialchars($news->content ?? '') ?></textarea>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card bg-black border-secondary mb-3">
                        ...
                        <div class="card-body">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" name="is_published" id="isPublished" <?= $news->is_published ? 'checked' : '' ?>>
                                <label class="form-check-label text-light" for="isPublished">Đã xuất bản</label>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <?php if ($news->image_url): ?>
                                    <div class="mb-2">
                                        <img src="<?= htmlspecialchars($news->image_url) ?>" alt="Thumbnail" class="img-fluid rounded border border-secondary">
                                    </div>
                                <?php endif; ?>
