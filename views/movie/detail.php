...
<!-- #9 Reviews Section -->
<div class="card bg-black border border-secondary rounded p-4 shadow-lg mt-4">
    <h3 class="h4 text-warning border-start border-warning border-4 ps-3 fw-bold mb-4">
        <i class="bi bi-star-fill me-2"></i>ĐÁNH GIÁ PHIM
    </h3>

    <?php if (isset($_SESSION['user_id'])): ?>
        <form method="POST" action="/reviews" class="mb-4 bg-dark p-3 rounded border border-secondary">
            <?= csrf_field() ?>
            <input type="hidden" name="movie_id" value="<?= $movie->id ?>">
            <div class="mb-3">
                <label class="form-label text-light">Đánh giá của bạn</label>
                <select name="rating" class="form-select bg-black text-warning border-secondary" style="width: auto;">
            ...
            <div class="mb-3">
                <textarea name="comment" class="form-control bg-black text-light border-secondary" rows="3" placeholder="Chia sẻ cảm nghĩ của bạn về bộ phim này..." required></textarea>
            </div>
            <button type="submit" class="btn btn-warning"><i class="bi bi-send me-1"></i> Gửi đánh giá</button>
        </form>
    <?php else: ?>
        <div class="alert bg-dark text-light border-secondary">
            ...
            <?php foreach ($reviews as $review): ?>
                <div class="review-item border-bottom border-secondary pb-3 mb-3">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <strong class="text-light"><i class="bi bi-person-circle me-2 text-secondary"></i><?= htmlspecialchars($review['user_name']) ?></strong>
                    ...
                    <p class="text-light-50 mb-0" style="color: #adb5bd;"><?= nl2br(htmlspecialchars($review['comment'])) ?></p>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
